...
const getEntryById = async (req, res) => {
   try {
      ...
      if (entry.user_id !== req.user.user_id) {
         logger.warn(
            `Unauthorized access attempt: User ${req.user.user_id} tried to access entry ${req.params.id}`
         );
      ...
      logger.info(
         `Returning entry with ID ${req.params.id} (Date: ${entry.entry_date})`
      );
...
const postEntry = async (req, res, next) => {
   try {
      ...
      logger.info(
         `Creating a new entry for user ${req.user.user_id} (Date: ${normalizedDate})`
      );
...
const updateEntryById = async (req, res) => {
   try {
      ...
      if (existingEntry.user_id !== req.user.user_id) {
         logger.warn(
            `Unauthorized update attempt: User ${req.user.user_id} tried to update entry ${req.params.id}`
         );
         return res.status(403).json({ message: "Not authorized" });
      }
      ...
      // Log update details
      logger.info(
         `Updating entry with ID ${req.params.id}, data:`,
         updatedData
      );
...
const deleteEntryById = async (req, res) => {
   try {
      ...
      if (existingEntry.user_id !== req.user.user_id) {
         logger.warn(
            `Unauthorized deletion attempt: User ${req.user.user_id} tried to delete entry ${req.params.id}`
         );
         return res.status(403).json({ message: "Not authorized" });
      }
