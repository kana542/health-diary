...
const insertEntry = async (entry) => {
  try {
    ...
    logger.actionInfo('CREATE', 'Entry', result.insertId, `(Date: ${entryDate})`);
...
const selectEntryById = async (entryId) => {
  try {
    ...
    if (rows[0]) {
      logger.info(`Retrieved entry with ID ${entryId} (Date: ${rows[0].entry_date})`);
    } else {
...
const updateEntry = async (entryId, entryData) => {
  try {
    ...
    if (result.affectedRows > 0) {
      logger.actionInfo('UPDATE', 'Entry', entryId, updateDetails.join(', '));
