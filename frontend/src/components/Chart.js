...
export class ImprovedChart {
         ...
         case "weight":
            if (this.entries && this.entries.length > 0) {
               const validEntries = this.entries.filter(
                  (entry) => entry.weight !== null && entry.weight !== undefined
               );
               ...
               if (validEntries.length > 0) {
                  ...
                  const padding = Math.max(
                     5,
                     Math.ceil((maxWeight - minWeight) * 0.1)
                  );
   ...
   static updateChart() {
      ...
      this.chartData = this.getMonthData(
         this.currentYear,
         this.currentMonth,
         this.currentMetric
      );
      ...
      // Update chart title
      this.chart.data.datasets[0].label = this.getMetricLabel(
         this.currentMetric
      );
   ...
   static getMonthData(year, month, metric) {
      ...
      const monthEntries = this.entries.filter((entry) => {
         ...
         const entryDate = new Date(
            `${DateUtils.toISODate(entry.entry_date)}T12:00:00Z`
         );
         return (
      ...
      console.log(
         `Chart: Fetching data for year ${year}, month ${month}, metric ${metric}`
      );
