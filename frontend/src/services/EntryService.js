...
class EntryService {
   ...
   async getAllEntries() {
      ...
      if (this.pendingRequest) {
         console.log(
            "EntryService: Request is already in progress, waiting..."
         );
   ...
   async getEntryById(id) {
      try {
         ...
         if (this.entriesCache) {
            const cachedEntry = this.entriesCache.find(
               (entry) => entry.entry_id === parseInt(id)
            );
            if (cachedEntry) {
               console.log(
                  `EntryService: Returning entry with ID ${id} from cache`
               );
               return cachedEntry;
            }
         }
   ...
   async createEntry(entryData) {
      try {
         ...
         if (formattedData.entry_date) {
            formattedData.entry_date = DateUtils.toISODate(
               formattedData.entry_date
            );
         }
