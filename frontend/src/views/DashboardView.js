...
export default class DashboardView {
   ...
   static async initialize() {
      ...
      if (this.isInitialized) {
         console.log(
            "DashboardView is already initialized, skipping initialization"
         );
         return;
      }
   ...
   static setupEventListeners() {
      ...
      eventBus.subscribe("entry:selected", (entry) =>
         this.showEntryModal(entry)
      );
      eventBus.subscribe("date:selected", (date) =>
         this.showNewEntryModal(date)
      );
      eventBus.subscribe("entries:list", (data) =>
         this.showEntriesListModal(data.date, data.entries)
      );
   }
   ...
   static setupEntryForm() {
      ...
      if (form && !form.hasAttribute("data-initialized")) {
         ...
         if (form.hasSubmitListener) {
            ...
            console.log(
               "Form submit handler is already registered, skipping registration"
            );
            return;
         }
   ...
   static updateMoodIcon(value) {
      ...
      const moodMap = [
         {
            value: 1,
            icon: '<box-icon name="sad" type="solid" color="#d62828"></box-icon>',
            label: "Sad",
            code: "Sad",
         },
         {
            value: 2,
            icon: '<box-icon name="tired" type="solid" color="#f77f00"></box-icon>',
            label: "Tired",
            code: "Tired",
         },
         {
            value: 3,
            icon: '<box-icon name="confused" type="solid" color="#ffd166"></box-icon>',
            label: "Neutral",
            code: "Neutral",
         },
         {
            value: 4,
            icon: '<box-icon name="smile" type="solid" color="#5fa8d3"></box-icon>',
            label: "Satisfied",
            code: "Satisfied",
         },
         {
            value: 5,
            icon: '<box-icon name="happy" type="solid" color="#38b000"></box-icon>',
            label: "Happy",
            code: "Happy",
         },
      ];
      ...
      const mood =
         moodMap.find((m) => m.value === parseInt(value)) || moodMap[2];
   ...
   static async saveEntry() {
      try {
         ...
         this.showSuccessNotification(
            entryId ? "Entry updated" : "New entry added"
         );
      } catch (error) {
